  ...
  m_width(width), m_height(height), m_player(player), m_world(world)
{
  ...
  if (FAILED(hr))
  {
    ErrorLogger::Log(hr, L"Ei nyt toiminu t‰‰ valobufferin teko");
  }
  ...
  if (FAILED(hr))
  {
    ErrorLogger::Log(hr, L"Ei nyt toiminu t‰‰ samplerstaten teko");
  }

  hr = DirectX::CreateWICTextureFromFile(m_device.Get(), L"Textures/grass.bmp", NULL, &m_textureView);
  if (FAILED(hr))
  {
    ErrorLogger::Log(hr, L"Ei nyt toiminu t‰‰ tekstuurin teko");
  }
...
void GraphicsEngine::InitCamera()
{
  ...
  if (FAILED(hr))
  {
    ErrorLogger::Log(hr, L"Ei nyt toiminu t‰‰ kamerabufferin teko");
  }
...
void GraphicsEngine::RenderFrame(void)
{
  ...
  if (FAILED(hr))
  {
    ErrorLogger::Log(hr, L"Ei nyt toiminu t‰‰ present");
  }
}
